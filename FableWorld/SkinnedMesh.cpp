...
{
	if(FAILED(D3DXCreateEffectFromFile(pDxDevice,"shaders/SkinnedMeshShader.fx",0,0,D3DXSHADER_DEBUG,0,&m_pEffect,0)))
	{
...
{
	D3DXCreateEffectFromFile(pDxDevice, "shaders/Text3DShader.fx", 0, 0, D3DXSHADER_DEBUG, 0, &m_pTitlesEffect, 0);
...
void SkinnedMesh::OnRender()
{
	...
	D3DXMATRIX MeshCombinedMatrix = S*(R*R1)*T;

	m_pEffect->SetTechnique(m_hEffectTechnique);
			
	m_pEffect->SetMatrix(m_hWVPMatrix,&(MeshCombinedMatrix * (camera->GetViewProjMatrix())));
	...
	if( pDinput->IsMouseButtonDown(0) )
	{
		...
		AABB box;
		m_BoundingBox.transformByMatrix(MeshCombinedMatrix, box);
