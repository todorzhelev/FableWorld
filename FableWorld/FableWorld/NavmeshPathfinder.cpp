...
#include "../CORE/Recast/Detour/DetourCommon.h"
...
void NavmeshPathfinder::init(Navmesh* sample)
{
	m_sample = sample;
	...
	if (m_navQuery)
	{
		...
		m_filter.setAreaCost(NAVMESH_POLYAREA_ROAD, 1.0f);
		m_filter.setAreaCost(NAVMESH_POLYAREA_DOOR, 1.0f);
		m_filter.setAreaCost(NAVMESH_POLYAREA_GRASS, 2.0f);
		m_filter.setAreaCost(NAVMESH_POLYAREA_JUMP, 1.5f);
	}
...
void NavmeshPathfinder::handleMenu()
{
...
void NavmeshPathfinder::handleClick(const float* /*s*/, const float* p, bool shift)
{
	if (shift)
	{
		m_sposSet = true;
...
void NavmeshPathfinder::handleStep()
{
}

void NavmeshPathfinder::handleToggle()
{
...
void NavmeshPathfinder::handleUpdate(const float /*dt*/)
{
...
void NavmeshPathfinder::handleRender()
{
...
void NavmeshPathfinder::handleRenderOverlay(double* proj, double* model, int* view)
{
	//GLdouble x, y, z;
...
void NavmeshPathfinder::drawAgent(const float* pos, float r, float h, float c, const unsigned int col)
{
	//duDebugDraw& dd = m_sample->getDebugDraw();
...
void NavmeshPathfinder::FindPath(float startPosition[3], float endPosition[3])
{
	...
	m_spos[0] = startPosition[0];
	m_spos[1] = startPosition[1];
	m_spos[2] = startPosition[2];

	m_epos[0] = endPosition[0];
	m_epos[1] = endPosition[1];
	m_epos[2] = endPosition[2];
...
std::vector<D3DXVECTOR3> NavmeshPathfinder::GetCalculatedPath()
{
	std::vector<D3DXVECTOR3> path;

	for (int i = 0; i < m_nsmoothPath; ++i)
	{
		//correction directly from here. x and y axis are inversed
		D3DXVECTOR3 vec;
		vec.x = -m_smoothPath[i * 3];
		vec.y = -m_smoothPath[i * 3 + 1];
		vec.z = m_smoothPath[i * 3 + 2];
