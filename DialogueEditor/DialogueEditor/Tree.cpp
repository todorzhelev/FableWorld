...
node* Tree::InsertNode(node* pNode,string text,int val,float x,float y,string quest)
{
	if( pNode == NULL )
	{
		int nWidth = static_cast<int>(GetStringWidth(text));
		int nHeight = static_cast<int>(GetStringHeight(text));

		pNode			  = new node;
		pNode->m_pLabel   = new Label(text,false,nWidth,nHeight,D3DXVECTOR2(x,y));
		pNode->m_nKey	  = val;
	...
	{	
		...
		pNode->m_vNodes.back() = InsertNode(nullptr,text,val,x,y,quest);
